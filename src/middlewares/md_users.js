...
const registerFields = async (req, res, next) => {
    ...
    try {
        await utils.validateNomeEmailSenha.validate({ email, nome, senha });
...
const loginFields = async (req, res, next) => {
    ...
    try {
        await utils.validateEmailSenha.validate({ email, senha });
        ...
        return res.status(500).json({ message: errorMessages.server, error: error.message });
    }
}
...
const editFields = async (req, res, next) => {
    ...
    try {
        await utils.validateNomeEmailSenha.validate({ nome, email, senha })
