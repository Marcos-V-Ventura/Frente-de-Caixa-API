...
const deleteProductById = async (req, res) => {
	...
	try {
		...
		if (findProductOrder) {
			return res.status(403).json({ mensagem: errorMessages.linkedProduct(findProductOrder.id) });
		}
...
const listProducts = async (req, res) => {
	...
	try {
		if (categoria_id) {
			const listProductsByCategory = await knex("produtos").where({ categoria_id, });
...
const detailProducts = async (req, res) => {
	...
	try {
		...
		if (!findProduct) {
			return res.status(400).json({ mensagem: errorMessages.productNotFound(id) });
		}
